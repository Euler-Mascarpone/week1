 ...
 *  - But not all are: Some methods doesn't take any input and some doesn't return any result (or both, rare).
 *
...
public class MethodsBasics {
    ...
    void program() {
        ...
        out.println(add(1, 3) == 4);    // Call method add, arguments 1 and 3
        out.println(f2c(32) == 0);  // Call method f2c
        out.println(abs(-12) == 12);
        out.println(pow(2, 8) == 256);
        out.println(isEven(6) == true);  // Call method isEven(), argument 6
    ...
    // - Parameter names may be chosen arbitrary, we need parameter
    // for incoming data from call (arguments (values) copied to parameters)
    int add(int a, int b) {
        int sum = a + b;        // sum not really needed, see below
        return sum;
    }
    ...
    // Expression n % 2 == 0 evaluated, then result sent back, no need for result variable
